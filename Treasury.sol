...
contract GravTreasury is Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        require(_GRAV != address(0));
        Grav = _GRAV;

        isReserveToken[_MIM] = true;
        reserveTokens.push(_MIM);

        //    isLiquidityToken[ _GRAVDAI ] = true;
        //    liquidityTokens.push( _GRAVDAI );
    ...
    function deposit(
        ...
        IERC20Mintable(Grav).mint(msg.sender, send_);
    ...
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOf(_token, _amount);
        IOHMERC20(Grav).burnFrom(msg.sender, value);
    ...
    function repayDebtWithOHM(uint256 _amount) external {
        ...
        IOHMERC20(Grav).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, Grav, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(Grav).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(Grav).totalSupply().sub(totalDebt));
    }
        ...
        returns (uint256 value_)
    {
        if (isReserveToken[_token]) {
            ...
            value_ = _amount.mul(10**IERC20(Grav).decimals()).div(
